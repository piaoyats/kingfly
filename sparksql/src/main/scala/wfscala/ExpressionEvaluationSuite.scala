...
class ExpressionEvaluationSuite extends FunSuite {
  ...
  test("literals") {
    // Literal meaning is字面, we can see it as a wrapper?
    // all literal eval will return the value of it no matter what is the row
    // can write a more better unittest for eval on a real example
  ...
  test("LIKE Non-literal Regular Expression") {
    //'a symbol=>DslSymbol;DslSymbol has string method
    // string method creates a new AttributeReference of type string
    // AttributeReference=>DslAttribute;DslAttribute has method at
    // method at return a BoundReference
